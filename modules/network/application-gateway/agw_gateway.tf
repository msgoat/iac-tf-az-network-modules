...
resource azurerm_application_gateway gateway {
  ...
  backend_address_pool {
    ...
    fqdns = var.aks_ingress_controller_dns != "" ? [var.aks_ingress_controller_dns] : null
    ip_addresses = var.aks_ingress_controller_ip != "" ? [var.aks_ingress_controller_ip] : null
  }

  backend_http_settings {
    name = local.backend_http_settings_name
    cookie_based_affinity = "Disabled"
    port = var.aks_ingress_controller_port
    protocol = title(var.aks_ingress_controller_protocol)
    request_timeout = 900
    probe_name = local.probe_name
    connection_draining {
      drain_timeout_sec = 900
      enabled           = true
    }
  }
  ...
  probe {
    ...
    protocol = title(var.aks_ingress_controller_probe_protocol)
    host = var.aks_ingress_controller_dns != "" ? var.aks_ingress_controller_dns : var.aks_ingress_controller_ip
    port = var.aks_ingress_controller_probe_port
    path = var.aks_ingress_controller_probe_path
    match {
      status_code = var.aks_ingress_controller_probe_status_codes
    }
    interval = 30
    timeout = 5
